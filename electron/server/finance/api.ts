...
export const createIncomeRecord = (record: Parameters<typeof db.createIncomeRecord>[0]) => db.createIncomeRecord(record);
export const updateIncomeRecord = (recordId: number, data: Parameters<typeof db.updateIncomeRecord>[1]) => db.updateIncomeRecord(recordId, data);
export const deleteIncomeRecord = (recordId: number) => db.deleteIncomeRecord(recordId);
...
const getReconcileStartPoint = async (record: ExpenseRecord) => {
  if (record.is_sub_record && record.parent_record_id) {
    const parentRecord = await getExpenseRecordWithID(record.parent_record_id);
    if (parentRecord) {
...
const reconcileExpenseRecord = async (record: ExpenseRecord, plans: ExpensePlan[]) => {
  const needReconciledRecord = await getReconcileStartPoint(record);
